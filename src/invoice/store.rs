...
impl InvoiceDetails {
	pub(crate) fn new(
		...
		Self {
			...
			created_at,
			expiry_secs,
			linked_payment_ids: None, // None at creation
...
impl Writeable for InvoiceDetails {
	fn write<W: lightning::util::ser::Writer>(
		...
		write_tlv_fields!(writer, {
			...
			(12, self.linked_payment_ids, option),  // Optional now
...
impl StorableObject for InvoiceDetails {
	...
	fn update(&mut self, update: &Self::Update) -> bool {
		...
		if let Some(payment_id) = update.add_payment_id {
...
mod tests {
	...
	fn invoice_details_update_with_payment() {
		...
		let update = InvoiceDetailsUpdate { id: invoice_id, add_payment_id: Some(payment_id) };
	...
	fn invoice_details_bolt12_offer_multiple_payments() {
		...
		let update1 = InvoiceDetailsUpdate { id: invoice_id, add_payment_id: Some(payment_id1) };
		invoice.update(&update1);

		let update2 = InvoiceDetailsUpdate { id: invoice_id, add_payment_id: Some(payment_id2) };
