...
use payjoin::receive::v2::{
	...
	PayjoinProposal, ProvisionalProposal, ReceiveSession, Receiver, ReceiverBuilder,
...
pub(crate) struct PayjoinManager {
	...
	receiver_persister: Mutex<KVStorePayjoinReceiverPersister>,
	logger: Arc<Logger>,
	config: Arc<Config>,
...
impl PayjoinManager {
	pub(crate) fn new(
		payjoin_session_store: Arc<PayjoinSessionStore>,
		receiver_persister: KVStorePayjoinReceiverPersister, logger: Arc<Logger>,
		config: Arc<Config>, broadcaster: Arc<Broadcaster>, wallet: Arc<Wallet>,
		fee_estimator: Arc<OnchainFeeEstimator>, chain_source: Arc<ChainSource>,
		stop_receiver: tokio::sync::watch::Receiver<()>,
	) -> Self {
		let receiver_persister = Mutex::new(receiver_persister);
		Self {
	...
	async fn receive_payjoin(
		...
		let locked_persister = self.receiver_persister.lock().unwrap();
			...
			let state_transition = session
				.process_response(
					ohttp_response
						...
						.await
						.map_err(|e| {
							log_error!(self.logger, "Failed to read response bytes: {}", e);
							Error::InvalidAddress
						})?
						.to_vec()
						.as_slice(),
	...
	async fn post_request(&self, req: payjoin::Request) -> Result<reqwest::Response, Error> {
	...
	async fn handle_error(
		...
		let err_response = match self.post_request(err_req).await {
			...
			Err(e) => return Err(Error::InvalidAddress),
		};
